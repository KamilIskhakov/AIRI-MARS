...
Для режима `marked_pair` каждый текст обрезается вокруг отмеченного целевого
упоминания. Поэтому `[E1]...[/E1]` и `[E2]...[/E2]` остаются во входе даже для
документов, которые значительно длиннее `MAX_LENGTH`.
